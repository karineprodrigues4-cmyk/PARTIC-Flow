...
function Members() {
  ...
  const [filters, setFilters] = useState({ city: "", spec: "", sub: "", search: "" });
  const [showAdd, setShowAdd] = useState(false);
  const [editM, setEditM] = useState(null);
  const [form, setForm] = useState({ name: "", profession: "Médico", specialty: "", subspecialty: "", city: "", state: "", personalContact: "", schedulingContact: "", email: "", instagram: "", monthlyFee: 297, status: "active", isAnchor: false, notes: "" });
  const save = d => { setMembers(d); DB.set("members", d); };

  const cities = [...new Set(members.map(m => m.city))].filter(Boolean).sort();
  const specs = [...new Set(members.map(m => m.specialty))].filter(Boolean).sort();
  const filtered = members.filter(m => {
    if (filters.city && m.city !== filters.city) return false;
    if (filters.spec && m.specialty !== filters.spec) return false;
    if (filters.search && !m.name.toLowerCase().includes(filters.search.toLowerCase())) return false;
    return true;
  });

  function openAdd() { setForm({ name: "", profession: "Médico", specialty: "", subspecialty: "", city: "", state: "", personalContact: "", schedulingContact: "", email: "", instagram: "", monthlyFee: 297, status: "active", isAnchor: false, notes: "" }); setEditM(null); setShowAdd(true); }
  function openEdit(m) { setForm({ ...m }); setEditM(m); setShowAdd(true); }
  function saveMember() {
    if (!form.name) return alert("Nome é obrigatório");
    if (editM) { save(members.map(m => m.id === editM.id ? { ...m, ...form } : m)); showToast("Membro atualizado ✓"); }
    else { save([...members, { ...form, id: Date.now(), joinedAt: new Date().toISOString() }]); showToast("Membro adicionado ✓"); }
    setShowAdd(false);
  }
    ...
    <div>
      <div style={S.topbar}>
        <div>
          ...
          <p style={S.sectionSub}>{members.filter(m => m.status === "active").length} ativos · {members.length} total</p>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button style={S.btnO} onClick={exportCSV}>{I.export} Exportar CSV</button>
          <button style={S.btnP} onClick={openAdd}>{I.plus} Novo membro</button>
        </div>
      </div>

      <FilterBar cities={cities} specialties={specs} onFilter={setFilters} />

      <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#f7f7f5" }}>
              {["Profissional", "Especialidade", "Cidade", "Tel. Pessoal", "Tel. Agendamento", "E-mail", "Status", ""].map(h => <th key={h} style={S.th}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtered.map(m => (
              <tr key={m.id} style={{ borderBottom: "1px solid #f5f5f5" }}>
                <td style={S.td}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <Avatar name={m.name} size={30} city={m.city} />
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{m.name}{m.isAnchor && <span title="Âncora" style={{ marginLeft: 5, color: C.verdeMedio, fontSize: 11 }}>⚓</span>}</div>
                      <div style={{ fontSize: 11, color: "#888" }}>{m.profession}</div>
                    </div>
                  </div>
                </td>
                <td style={S.td}>
                  <div style={{ fontSize: 12 }}>{m.specialty}</div>
                  {m.subspecialty && <div style={{ fontSize: 11, color: "#EF4444", fontWeight: 600 }}>{m.subspecialty}</div>}
                </td>
                <td style={S.td}><span style={{ fontSize: 11, background: getCityColor(m.city) + "20", color: getCityColor(m.city), borderRadius: 99, padding: "2px 8px", fontWeight: 700 }}>{m.city}</span></td>
                <td style={S.td}>
                  {m.personalContact ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ fontSize: 12 }}>{m.personalContact}</span>
                      <a href={`https://wa.me/55${m.personalContact.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" style={{ color: "#128C7E" }}>{I.wpp}</a>
                    </div>
                ...
                <td style={S.td}>
                  {m.schedulingContact ? (
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ fontSize: 12 }}>{m.schedulingContact}</span>
                      <a href={`https://wa.me/55${m.schedulingContact.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" style={{ color: "#128C7E" }}>{I.wpp}</a>
                    </div>
                  ) : <span style={{ color: "#ddd" }}>—</span>}
                </td>
                <td style={S.td}>
                  {m.email ? <a href={`mailto:${m.email}`} style={{ ...S.btnSm(C.azulPetroleo), textDecoration: "none", fontSize: 11 }}>{I.mail}</a> : <span style={{ color: "#ddd" }}>—</span>}
                </td>
                <td style={S.td}>
                  <button onClick={() => save(members.map(x => x.id === m.id ? { ...x, status: x.status === "active" ? "inactive" : "active" } : x))}
                    style={{ background: m.status === "active" ? "#f0faf4" : "#f7f7f5", color: m.status === "active" ? "#0f6e56" : "#888", border: `1px solid ${m.status === "active" ? "#a7f0d8" : "#e5e5e5"}`, borderRadius: 99, padding: "3px 10px", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                    {m.status === "active" ? "● Ativo" : "○ Inativo"}
                  </button>
                </td>
                <td style={S.td}>
                  <div style={{ display: "flex", gap: 5 }}>
                    <button style={S.btnO} onClick={() => openEdit(m)}>{I.edit}</button>
                    <button style={S.btnD} onClick={() => { if (confirm("Remover?")) save(members.filter(x => x.id !== m.id)); }}>{I.trash}</button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && <tr><td colSpan={8} style={{ padding: "40px 20px", textAlign: "center", color: "#aaa" }}>Nenhum membro encontrado</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showAdd && (
        <div style={S.modal} onClick={() => setShowAdd(false)}>
          <div style={{ ...S.modalBox, maxWidth: 580 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: 16, fontWeight: 800, margin: "0 0 18px" }}>{editM ? "Editar Membro" : "Novo Membro"}</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
              <div style={{ gridColumn: "1/-1" }}><label style={S.lbl}>Nome *</label><input style={S.input} value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} /></div>
              <div><label style={S.lbl}>Profissão</label><select style={S.select} value={form.profession} onChange={e => setForm(p => ({ ...p, profession: e.target.value }))}>{["Médico", "Nutricionista", "Psicólogo", "Fisioterapeuta", "Fonoaudiólogo", "Outro"].map(o => <option key={o}>{o}</option>)}</select></div>
              <div><label style={S.lbl}>Especialidade</label><input style={S.input} value={form.specialty} onChange={e => setForm(p => ({ ...p, specialty: e.target.value }))} /></div>
              <div><label style={S.lbl}>Subespecialidade</label><input style={S.input} value={form.subspecialty} onChange={e => setForm(p => ({ ...p, subspecialty: e.target.value }))} /></div>
              <div><label style={S.lbl}>Cidade</label><input style={S.input} value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} /></div>
              <div><label style={S.lbl}>Estado</label><input style={S.input} value={form.state} onChange={e => setForm(p => ({ ...p, state: e.target.value }))} maxLength={2} /></div>
              <div><label style={S.lbl}>Tel. Pessoal (WPP)</label><input style={S.input} value={form.personalContact} onChange={e => setForm(p => ({ ...p, personalContact: e.target.value }))} /></div>
              <div><label style={S.lbl}>Tel. Agendamento (Secretária)</label><input style={S.input} value={form.schedulingContact} onChange={e => setForm(p => ({ ...p, schedulingContact: e.target.value }))} /></div>
              <div><label style={S.lbl}>E-mail</label><input style={S.input} value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} /></div>
              <div><label style={S.lbl}>Instagram</label><input style={S.input} value={form.instagram} onChange={e => setForm(p => ({ ...p, instagram: e.target.value }))} /></div>
              <div><label style={S.lbl}>Mensalidade (R$)</label><input style={S.input} type="number" value={form.monthlyFee} onChange={e => setForm(p => ({ ...p, monthlyFee: Number(e.target.value) }))} /></div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 20 }}>
                <input type="checkbox" id="anc" checked={form.isAnchor} onChange={e => setForm(p => ({ ...p, isAnchor: e.target.checked }))} />
                <label htmlFor="anc" style={{ fontSize: 13, cursor: "pointer" }}>⚓ Profissional âncora</label>
              </div>
              <div style={{ gridColumn: "1/-1" }}><label style={S.lbl}>Observações</label><textarea style={{ ...S.input, height: 60, resize: "vertical" }} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} /></div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button style={S.btnP} onClick={saveMember}>{editM ? "Atualizar" : "Salvar"}</button>
              <button style={S.btnO} onClick={() => setShowAdd(false)}>Cancelar</button>
            </div>
          </div>
        </div>
